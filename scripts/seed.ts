...
async function seed() {
  ...
  const userDefs = [
    { name: "Alexandre Martin",  email: "alexandre.martin@ij-pdl.fr",  role: "admin_ij_pdl"   as const, structure: "IJ Pays de la Loire",       phone: "02 40 00 00 01" },
    { name: "Marie Leroy",       email: "marie.leroy@ij-nantes.fr",     role: "salarie_ij_pdl" as const, structure: "IJ Nantes Métropole",        phone: "02 40 00 00 02" },
    { name: "Thomas Dubois",     email: "thomas.dubois@ij-49.fr",       role: "salarie_ij_pdl" as const, structure: "IJ Maine-et-Loire",          phone: "02 41 00 00 03" },
    { name: "Sophie Bernard",    email: "sophie.bernard@pij-85.fr",     role: "pro_reseau_ij"  as const, structure: "PIJ La Roche-sur-Yon",       phone: "02 51 00 00 04" },
    { name: "Sarah Lemoine",     email: "sarah.lemoine@crij-pdl.fr",    role: "pro_reseau_ij"  as const, structure: "CRIJ Pays de la Loire",      phone: "02 40 00 00 05" },
    { name: "Marc Durand",       email: "marc.durand@bij-laval.fr",     role: "pro_reseau_ij"  as const, structure: "BIJ Laval",                  phone: "02 43 00 00 06" },
    { name: "Claire Petit",      email: "claire.petit@ml44.fr",         role: "relais_externe" as const, structure: "Mission Locale 44",          phone: "02 40 00 00 07" },
    { name: "Lucas Moreau",      email: "lucas.moreau@cidj.com",        role: "relais_externe" as const, structure: "CIDJ Paris",                 phone: "01 44 49 12 00" },
  ]
  ...
  const resourceDefs: {
  ...
  }[] = [
    {
      ...
      description: "Guide complet des éléments visuels, couleurs, typographies et règles d'usage de la marque Info Jeunes PDL.",
      type: "documentation",
      status: "published",
      pinned: true,
      authorId: adminId,
    ...
    },
    {
      ...
      description: "Nouveaux barèmes et dates clés pour les dépôts de dossiers de subvention. Mis à jour pour l'exercice 2024.",
      type: "documentation",
      status: "published",
      pinned: true,
      authorId: sarahId,
    ...
    },
    {
      ...
      description: "Pack complet : affiches, visuels réseaux sociaux, modèles email et guide de déploiement local pour la campagne estivale.",
      type: "toolbox",
      status: "published",
      pinned: false,
      authorId: marieId,
    ...
    },
    {
      ...
      description: "Vidéo pas-à-pas pour remplir le formulaire de rapport d'activité dans Hub Pro. Durée : 8 minutes.",
      type: "toolbox",
      status: "published",
      pinned: false,
      authorId: adminId,
    ...
    },
    {
      ...
      description: "Analyse des impacts de la réforme 2023-2024 sur les dispositifs d'orientation pour les 15-25 ans.",
      type: "veille",
      status: "published",
      pinned: false,
      authorId: sarahId,
    ...
    },
    {
      ...
      description: "Tour d'horizon des appels à projets Erasmus+ ouverts pour les structures Info Jeunes en 2024.",
      type: "veille",
      status: "published",
      pinned: false,
      authorId: marcId,
    ...
    },
    {
      ...
      description: "Tutoriel d'introduction complet pour les nouveaux utilisateurs de la plateforme.",
      type: "tutorial",
      status: "published",
      pinned: false,
      authorId: adminId,
    ...
    },
    {
      ...
      description: "Comment rédiger, illustrer et publier une actualité visible par tout le réseau IJ PDL.",
      type: "tutorial",
      status: "published",
      pinned: false,
      authorId: marieId,
  ...
  const eventDefs: {
  ...
  }[] = [
    {
      ...
      description: "Point mensuel des coordinateurs IJ Pays de la Loire. Ordre du jour : bilan campagne Job d'Été, préparation séminaire automne, questions diverses.",
    ...
    },
    {
      ...
      description: "Journée de formation sur les dispositifs Erasmus+ pour les structures IJ. Présentation des appels à projets ouverts en 2024 et accompagnement au montage de dossier.",
    ...
    },
    {
      ...
      description: "Repas convivial de l'équipe IJ Nantes Métropole. Bienvenue à nos nouvelles recrues !",
    ...
    },
    {
      ...
      description: "Webinaire national sur les impacts de la réforme de l'apprentissage pour les professionnels IJ. Intervenants : DARES, CIDJ, représentants régionaux.",
    ...
    },
    {
      ...
      description: "Grande journée annuelle du réseau IJ PDL. Ateliers thématiques, présentation des projets phares, remise des prix réseau. Inscription obligatoire.",
  ...
  await db.insert(schema.channels).values([
    { id: generalId,    name: "général",    description: "Canal principal du réseau IJ PDL", type: "channel", createdById: adminId },
    { id: formationsId, name: "formations", description: "Échanges sur les formations et dispositifs", type: "channel", createdById: adminId },
  ])

  // Members
  const allUserIds = Object.values(userIds)
  for (const uid of allUserIds) {
    await db.insert(schema.channelMembers).values({ channelId: generalId, userId: uid })
  }
  for (const uid of [adminId, marieId, thomasId, sarahId, marcId]) {
    await db.insert(schema.channelMembers).values({ channelId: formationsId, userId: uid })
  }

  // Messages — #général
  const generalMessages = [
    { authorId: marieId,  body: "Bonjour à tous ! 👋 Bienvenue sur Hub Pro. N'hésitez pas à explorer les différentes sections.", createdAt: daysAgo(5) },
    { authorId: thomasId, body: "Merci pour l'accès ! Je viens de trouver le guide des dispositifs régionaux dans la boîte à outils, très utile.", createdAt: daysAgo(4) },
    { authorId: sophieId, body: "Est-ce que tu as fini le dossier pour l'appel à projets « Été Jeunes » Thomas ?", createdAt: hoursAgo(3) },
    { authorId: thomasId, body: "Presque ! Je l'envoie ce soir. On se voit à la réunion de coordination la semaine prochaine ?", createdAt: hoursAgo(2) },
  ]

  // Messages — #formations
  const formationsMessages = [
    { authorId: sarahId, body: "Je mets en ligne le programme détaillé du webinaire sur la réforme de l'apprentissage. Pensez à vous inscrire !", createdAt: daysAgo(2) },
    { authorId: marcId,  body: "Merci Sarah. Est-ce qu'il y aura un replay disponible pour ceux qui ne peuvent pas y assister en direct ?", createdAt: daysAgo(1) },
    { authorId: sarahId, body: "Oui, le replay sera disponible dans la boîte à outils 48h après la session.", createdAt: hoursAgo(6) },
  ]

  for (const m of generalMessages) {
    await db.insert(schema.chatMessages).values({ id: createId(), channelId: generalId, authorId: m.authorId, body: m.body, createdAt: m.createdAt })
  }
  for (const m of formationsMessages) {
    await db.insert(schema.chatMessages).values({ id: createId(), channelId: formationsId, authorId: m.authorId, body: m.body, createdAt: m.createdAt })
  }

  console.log(`✓ 2 channels, ${generalMessages.length + formationsMessages.length} messages`)
}
